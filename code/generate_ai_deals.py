...
def run_gold_ai():
    ...
    if not os.path.exists(silver_path):
        ...
        if os.path.exists("vehicles_gold.json"):
            logging.info("Parquet not found, generating from vehicles_gold.json...")
            with open("vehicles_gold.json", "r", encoding="utf-8") as f:
                raw = json.load(f)
